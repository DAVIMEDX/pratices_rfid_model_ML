...
model_py = f"""# model.py — AUTO-GERADO
import math
import array
...
W1=array.array('b', {fl(W1q)})
B1=array.array('b', {fl(b1q)})
W2=array.array('b', {fl(W2q)})
B2=array.array('b', {fl(b2q)})
WD=array.array('b', {fl(Wdq)})
BD=array.array('b', {fl(bdq)})
WO=array.array('b', {fl(Woq)})
BO=array.array('b', {fl(boq)})
...
def _lstm_seq(seq, W, B, sW, sB):
    ...
    for x in seq:
        xh=x+h
        g=[sum(W[(gi*H+j)*(IN+H)+i]*xh[i] for i in range(len(xh)))*sW+B[gi*H+j]*sB for gi in range(4) for j in range(H)]
        nh=[]; nc=[]
        for j in range(H):
...
# Compilação Automática (Opcional, mas recomendada)
print("\n[5/5] A compilar para bytecode (.mpy) com mpy-cross...")
import subprocess
try:
    subprocess.run(["mpy-cross", str(OUTPUT_DIR / "model.py")], check=True)
    print(f"  Sucesso! Gerado binário otimizado: {OUTPUT_DIR}/model.mpy")
    print("\n" + "="*55)
    print("  Subir para a placa:")
    print("  mpremote cp src/model/model.mpy :model.mpy")
    print("="*55)
except FileNotFoundError:
    print("  mpy-cross nao encontrado. Envie o .py normal.")
